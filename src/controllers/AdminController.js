...
const headingColumnsNames = ['Criado', 'usuario','Supervisor', 'Operacional', "Nome","CPF",	"Nº Benefício","Tipo de Operação"	,"Banco Origem","Valor Parcela" ,"Quitação" ,	"Parcelas Restantes", "Contrato","Taxa","Status","Obs"]
...
function formulateSheetData(record,rowIndex){
  ...
  ws.cell(rowIndex, 7).string(String(record['cli_matricula']))       /* n beneficio */        
  ws.cell(rowIndex, 8).string(String(record['tipo_operacao']))        /*tipo de operacao  */
  ws.cell(rowIndex, 9).string(String(record['banco_origem']))       /* banco origem  */
  ws.cell(rowIndex, 10).string(String(record['v_parcela']))  /* valor parcelas */
  ws.cell(rowIndex, 11).string(String(record['quitacao']))  /* quitacao */
  ws.cell(rowIndex, 12).string(String(record['parcelas_restantes']))   /* parcelas restantes */
  ws.cell(rowIndex, 13).string(String(record['n_contrato']))     /* contrato */  
  ws.cell(rowIndex, 14).string(String(record['taxa']))        /* taxa */            
  ws.cell(rowIndex, 15).string(String(record['status']))         /* status */  
  ws.cell(rowIndex, 16).string(String(record['obs']))        /* obs */
}
